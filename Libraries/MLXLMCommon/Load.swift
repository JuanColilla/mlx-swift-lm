...
public struct WeightLoadingSelection: Sendable {
    ...
    public static func pipelineLayers(
        ...
        Self(
            ...
            rewrites: { key in
                guard let match = Self.layerMatch(in: key, prefixes: [destinationPrefix]) else {
                    return key
                }
    ...
    private static func layerMatch(in key: String, prefixes: [String]) -> (
        index: Int, suffix: String
    )? {
...
/// Weight files selected by a `WeightLoadingSelection`, for pipeline-parallel sharding.
///
/// Distinct from ``safetensorWeightURLs(in:selection:additionalFiles:)``: that chooses which
/// *files* hold the model's weights, this chooses which *files contain at least one included
/// tensor key*, since pipeline shards keep a subset of layer keys spread across all files.
package func pipelineWeightURLs(
    ...
    if FileManager.default.fileExists(atPath: indexURL.path) {
        ...
        return Set(
            index.weightMap.compactMap { key, filename in
...
///
/// - Parameters:
///   - lazy: when `true`, skip the final `eval(model)` so the caller can evaluate later
///   - selection: for pipeline-parallel sharding -- restricts loading to a subset of tensor
///     keys and optionally renames them, using ``pipelineWeightURLs(in:selection:)`` instead
///     of the concurrent loader so a rank never materializes layers it does not own
///   - weightFileSelection: how to choose the weight files when `selection` is `nil`, see
///     ``WeightFileSelection``
public func loadWeights(
    ...
    if let selection {
        // Pipeline-parallel sharding: filter per key while loading so a rank never
        // materializes tensors outside its local layer range.
        for url in try pipelineWeightURLs(in: modelDirectory, selection: selection) {
            let (w, m) = try loadArraysAndMetadata(url: url)
            for (key, value) in w where selection.includes(key) {
                weights[key] = value
            }
            if metadata.isEmpty {
                metadata = m
