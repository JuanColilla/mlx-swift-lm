...
public class Qwen35TextModelInner: Module {
    ...
    func forward(
        ...
        // El decode compilado se salta la ruta de pipeline: un rango > 0 reinterpretaria
        // los hidden states recibidos como ids de token.
        if pipelineRank == nil, applyFinalNorm, inputs.dim(1) == 1, let caches = cache,
...
public class Qwen35TextModel: Module, LLMModel, KVCacheDimensionProvider {
    ...
    public func callAsFunction(
        ...
        // Un shard de pipeline puede no contener ninguna capa de atencion completa
        // (`faIdx == nil`): sin ella no hay K/V compartido que ofrecer al drafter, y el
        // iterador MTP degrada a passthrough al no encontrar la clave "full_attention".
        if let faIdx = model.faIdx {
            outState[mtpSharedKVStatesKey] = qwen35SharedKVState(
                cache: cache, fullAttentionIndex: faIdx)
            outState[mtpSharedKVOffsetsKey] = qwen35SharedKVOffsets(
                cache: cache, fullAttentionIndex: faIdx)
            outState[mtpSharedKVSourceIndicesKey] = ["full_attention": faIdx]
        }
