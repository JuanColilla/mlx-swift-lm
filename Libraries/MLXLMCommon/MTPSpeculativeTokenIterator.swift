...
public struct MTPSpeculativeTokenIterator: TokenIteratorProtocol {
    ...
    mutating func prepare(input: LMInput, prefill: PrefillParameters = .init()) throws {
        ...
        // Qwen MTP necesita que el target emita una fila oculta por token del prompt;
        // el resto de drafters solo arrastra el estado de continuacion del turno previo.
        let incomingPrefillState: LMOutput.State? =
            drafter.requiresPromptPrefill ? prefillState : carriedState
